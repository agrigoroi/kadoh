...
var Node = module.exports = Peer.extend({
 ...
 //# INTERNAL EVENTS HANDLING
   ...
   */
   ...
   sendMessagePeer: function(peer, message, reactor, callback) {
    messageRPC = new MessageRPC(peer, message);
    reactor.sendRPC(messageRPC);
    Deferred.when(messageRPC).then(function(){callback(true)}, function(){callback(false)});
   },

   /**
   * TODO: write this thing!
   */
   sendMessage: function(node, message, callback) {
    var reactor = this._reactor;
    var sendMessagePeer = this.sendMessagePeer;
    this.iterativeFindNode(node).then(function(peers){
      sendMessagePeer(peers.getPeer(0), message, reactor, callback);
    }, function() {
      callback(false);
    });
   },

   handleBROADCAST: function(rpc) {
    var id = rpc.getID();
   ...
   broadcastToNeighboors: function(id, message) {
    var peers = this._routingTable.getClosePeers(this.getID(), this._routingTable.howManyPeers(), []);
   ...
   // Send a message to howManyPeers closest neighbors
   messageNeighbors: function(message, howManyPeers) {
    var peers = this._routingTable.getClosePeers(this.getID(), howManyPeers, []).toArray();
    for(var i=0;i<peers.length;i++) {
      var rpc = new MessageRPC(peers[i], message);
      this._reactor.sendRPC(rpc);
    }
   },
   ...
   */
   broadcast: function(message) {
    var id = Crypto.digest.randomSHA1().substring(0,2);
    this._broadcasts.push(id);
    this.broadcastToNeighboors(id, message);
   },
 ...
 //# ITERATIVE PROCESSES
   ...
   *   - resolve :
   *       {String}    key   - key with which the value was stored
